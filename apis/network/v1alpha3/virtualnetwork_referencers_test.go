...
const (
	mockName               = "mockName"
	mockVirtualNetworkName = "mockVirtualNetworkName"
)
...
func TestVirtualNetworkNameReferencerGetStatus(t *testing.T) {
	...
	for name, tc := range map[string]struct {
		...
		t.Run(name, func(t *testing.T) {
			r := VirtualNetworkNameReferencer{LocalObjectReference: corev1.LocalObjectReference{Name: mockName}}
...
func TestVirtualNetworkNameReferencerBuild(t *testing.T) {
	type input struct {
		readerFn func(ctx context.Context, key client.ObjectKey, obj runtime.Object) error
	}
	type expected struct {
	...
	for name, tc := range map[string]struct {
		...
		"ReferenceRetrieved_ReturnsExpected": {
			input: input{
				readerFn: func(ctx context.Context, key client.ObjectKey, obj runtime.Object) error {
					p := obj.(*VirtualNetwork)
					p.Spec.Name = mockVirtualNetworkName
					return nil
				},
			},
			expected: expected{
				value: mockVirtualNetworkName,
			},
		},
	} {
		t.Run(name, func(t *testing.T) {
